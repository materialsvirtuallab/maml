...
class RfxasXANESTest(unittest.TestCase):
    ...
    def test_option_validation(self):
        self.assertRaisesRegex(
            ValueError,
            "Invalid energy reference option",
            CenvPrediction,
            xanes_spectrum=self.xanes_obj,
            energy_reference="E1",
            energy_range=45,
            model_dir=model_dir,
        )
        self.assertRaisesRegex(
            ValueError,
            "range needs to be a number",
            CenvPrediction,
            xanes_spectrum=self.xanes_obj,
            energy_reference="lowest",
            energy_range=[-5, 45],
            model_dir=model_dir,
        )
        self.assertRaisesRegex(
            ValueError,
            "range needs to be larger than 0",
            CenvPrediction,
            xanes_spectrum=self.xanes_obj,
            energy_reference="lowest",
            energy_range=-20,
        ...
        self.assertRaisesRegex(
            ValueError,
            "lower bound needs to be less than zero",
            CenvPrediction,
            xanes_spectrum=self.xanes_obj,
            energy_reference="E0",
            energy_range=[10, 20],
        ...
        self.assertRaisesRegex(
            ValueError,
            "higher bound needs to be larger than zero",
            CenvPrediction,
            xanes_spectrum=self.xanes_obj,
            energy_reference="E0",
            energy_range=[-5, -2],
    ...
    def test_interpolation(self):
        ...
        self.assertTrue(
            np.allclose(
                pearsonr(self.xanes_obj_no_interp.interp_energy, self.xanes_obj_no_interp.xanes_spectrum.x)[0],
                1,
                rtol=1e-7,
            )
        )

        self.xanes_obj_interp = CenvPrediction(self.xanes_obj, "lowest", 45, None, True, model_dir=model_dir)
        self.assertTrue(
