...
class SelectKFromClusters(BaseEstimator, TransformerMixin):
    ...
    def transform(self, clustering_data: dict):
        ...
        elif "label_centers" not in clustering_data.keys():
            warnings.warn(
                "Centriod location is not provided, so random selection from each cluster will be performed, which will likely still significantly outperform manual sampling in terms of feature coverage."
            )
        ...
        for label in set(clustering_data["labels"]):
            ...
            if "label_centers" in clustering_data.keys():
                center_same_label = clustering_data["label_centers"][label]
                distance_to_center = np.sqrt(
                    np.sum((features_same_label - center_same_label) ** 2, axis=1)
                ).reshape(len(indexes_same_label), 1)
                select_k_indexes = [
                ...
                selected_indexes.extend(
                    indexes_same_label[
                        np.argpartition(
                            distance_to_center.sum(axis=1), select_k_indexes
                        )[select_k_indexes]
                    ]
                )
            else:
