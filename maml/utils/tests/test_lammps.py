...
from maml.utils._lammps import (stress_format_change, stress_list_to_matrix,
    check_structures_forces_stresses, write_data_from_structure)
...
class TestLAMMPS(unittest.TestCase):
    ...
    def test_check_structures(self):
        ...
        stress = np.array([-0.22279327, -1.2809575,  -0.44279698,
                           -0.23345818, -0.37798718, -0.17676364])

        checked_force = np.array([[0.05552151,  0.09063424,  0.05940176],
                                  [-0.05552151, -0.09063424, -0.05940176]])

        checked_stress = np.array([-0.26319715, -1.3219795, -0.3613719,
                                   -0.30627516, -0.27276486, -0.17306383])

        new_structures, new_forces, new_stresses = \
            check_structures_forces_stresses([s], [forces], [stress])
